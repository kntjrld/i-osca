...
$newbrgy = new Barangay();
$newbrgy->set_brgy($brgy[0]);

$trythis = $newbrgy->get_brgy();
...
$MaleAgeArr = array();
    ...
    $ages = "SELECT COUNT(*) as mage FROM tbl_records WHERE fx_barangay = '$testval' AND fx_gender = 'Male' AND fn_age BETWEEN $whereage[$i]";
...
$FemaleAgeArr = array();
    for ($i = 0; $i < count($whereage); $i++) {
    $ages = "SELECT COUNT(*) as fage FROM tbl_records WHERE fx_barangay = '$testval' AND fx_gender = 'Female' AND fn_age BETWEEN $whereage[$i]";
    $arrage = mysqli_query($conn,$ages);
...
$alive = array();
    for ($i = 0; $i < count($whereage); $i++) {
    $stat = "SELECT COUNT(*) as cnt FROM tbl_records WHERE fx_barangay = '$testval' AND life_status = '$life[0]' AND fn_age BETWEEN $whereage[$i]";
...
$dead = array();
    for ($i = 0; $i < count($whereage); $i++) {
    $stat = "SELECT COUNT(*) as cnt FROM tbl_records WHERE fx_barangay = '$testval' AND life_status = '$life[1]' AND fn_age BETWEEN $whereage[$i]";
    $arrstatus = mysqli_query($conn,$stat);
    $status=mysqli_fetch_assoc($arrstatus);
    $dead[] = $status['cnt'];
}
...
for ($i = 0; $i < count($agelabel); $i++) {
    $brgy1 .= "{ y:'$agelabel[$i]', Male:'$MaleAgeArr[$i]', Female:'$FemaleAgeArr[$i]', Alive:'$alive[$i]', Dead:'$dead[$i]'}, ";
    }
?>